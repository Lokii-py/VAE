...
#this will compute the average accuracy 
def compute_avg_accuracy(vae, data_loader, device):
    ...
    with torch.no_grad():
        for data, _ in data_loader:
            data = data.to(device)
            reconstructed = vae(data)  # Forward pass
            
            # Compute Mean Squared Error (MSE) per batch
            mse_loss = F.mse_loss(reconstructed, data, reduction="sum")
            total_mse += mse_loss.item()
            total_samples += data.size(0)

    # Compute average accuracy as 1 - (normalized MSE)
    avg_mse = total_mse / total_samples
    accuracy = 1 - avg_mse  # Lower MSE means better reconstructions

    print(f"Average Reconstruction Accuracy: {accuracy:.4f}")
...
for epoch in range(config.EPOCHS):
    ...
    # Compute and print average reconstruction accuracy
    compute_avg_accuracy(vae, test_loader, config.DEVICE)
